...
class im(SpuRequestHandler):
    ...
    def getContacts(self,
        ...
        sql = "select * from (select main.id,to_user_id,user.username to_user_name, max(create_on)create_on,(select content from im temp where temp.from_user_id = %s and temp.to_user_id = main.to_user_id order by id desc limit 1)content from im main left join user on user.id=main.to_user_id where from_user_id = %s group by to_user_id)t order by id desc" % (user_id, user_id)
